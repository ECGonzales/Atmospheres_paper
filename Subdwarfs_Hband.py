...
df_1256 = df_1256[(df_1256['w'] >= 1.49786) & (df_1256['w'] <= 1.79593)]
df_0532 = df_0532[(df_0532['w'] >= 1.51) & (df_0532['w'] <= 1.788)]
...
norm_region_0532 = df_0532[(df_0532['w'] >= 1.5) & (df_0532['w'] <= 1.52)]
norm_df_0532 = df_0532['f']/(np.average(norm_region_0532['f']))

# norm_region_0616 = df_0616[(df_0616['w'] >= 1.5) &(df_0616['w'] <= 1.52)]
# norm_df_0616 = df_0616['f']/(np.average(norm_region_0616['f']))

norm_region_1626 = df_1626[(df_1626['w'] >= 1.5) & (df_1626['w'] <= 1.52)]
norm_df_1626 = df_1626['f']/(np.average(norm_region_1626['f']))

norm_region_1444 = df_1444[(df_1444['w'] >= 1.5) & (df_1444['w'] <= 1.52)]
norm_df_1444 = df_1444['f']/(np.average(norm_region_1444['f']))

norm_region_1013 = df_1013[(df_1013['w'] >= 1.5) & (df_1013['w'] <= 1.52)]
norm_df_1013 = df_1013['f']/(np.average(norm_region_1013['f']))

norm_region_LHS = df_LHS[(df_LHS['w'] >= 1.5) & (df_LHS['w'] <= 1.52)]
norm_df_LHS = df_LHS['f']/(np.average(norm_region_LHS['f']))

norm_region_1425 = df_1425[(df_1425['w'] >= 1.5) & (df_1425['w'] <= 1.52)]
norm_df_1425 = df_1425['f']/(np.average(norm_region_1425['f']))

norm_region_1610 = df_1610[(df_1610['w'] >= 1.5) & (df_1610['w'] <= 1.52)]
norm_df_1610 = df_1610['f']/(np.average(norm_region_1610['f']))

norm_region_HD = df_HD[(df_HD['w'] >= 1.5) & (df_HD['w'] <= 1.52)]
norm_df_HD = df_HD['f']/(np.average(norm_region_HD['f']))

norm_region_2036 = df_2036[(df_2036['w'] >= 1.5) & (df_2036['w'] <= 1.52)]
norm_df_2036 = df_2036['f']/(np.average(norm_region_2036['f']))

# norm_region_125614 = df_125614[(df_125614['w'] >= 1.5) & (df_125614['w'] <= 1.52)]
...
plt.savefig('Plots/Subdwarfs_Hbandstack.png')