...
# -------- Remove connecting lines from SEDkit for field -------------------------------
df_field1 = df_field[(df_field['w'] <= 1.0324)]
df_field2 = df_field[(df_field['w'] >= 1.038)]
# Look at dataframe to get index values to slice the flux array
norm_df_field1 = norm_df_field[:3745]
norm_df_field2 = norm_df_field[3752:]
...
ax1.plot(df_field1['w'], norm_df_field1 + 1, c='#7C7D70')
ax1.plot(df_field2['w'], norm_df_field2 + 1, c='#7C7D70')
...
ax1.text(0.075, 0.025, 'Ti I', transform=ax1.transAxes, color='k', fontsize=15)
...
ax1.text(0.272, 0.76, 'FeH', transform=ax1.transAxes, color='k', fontsize=15)
# -- To make a vertical line
FeH1d = pd.DataFrame()
...
ax1.text(0.55, 0.86, 'FeH', transform=ax1.transAxes, color='k', fontsize=15)
FeH2d = pd.DataFrame()
FeH2d['x'] = [0.998, 0.998]
FeH2d['y'] = [3.7, 3.85]
...
ax1.text(0.9, 0.925, 'H$_\mathrm{2} $O', transform=ax1.transAxes, color='k', fontsize=15)
H2Od = pd.DataFrame()
H2Od['x'] = [1.08, 1.08]
H2Od['y'] = [3.95, 4.1]
plt.plot(H2Od['x'], H2Od['y'], color='k')

plt.savefig('Plots/YbandLbol.png')
