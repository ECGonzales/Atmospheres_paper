...
df_1256 = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/Gaia1256-0224 (L3.5sd) SED_smoothed.txt',
                      sep=" ", comment='#', header=None, names=["w", "f", "err"])

# -------------- Subdwarfs ----------------------------------
df_0532 = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/SDSS_phot0532+8246 (L7sd) SED_smoothed.txt', sep=" ",
                      comment='#', header=None, names=["w", "f", "err"])
df_0616 = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/X-shooter0616-6407 (L5sd) SED_smoothed.txt', sep=" ",
                      comment='#', header=None, names=["w", "f", "err"])
df_1013 = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/Gaia1013-1356 (M9.5sd) SED_smoothed.txt', sep=" ",
                      comment='#', header=None, names=["w", "f", "err"])
# df_125614 = pd.read_csv('', sep=" ", comment='#', header=None, names=["w", "f", "err"])
# df_HD = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/HD114762B (M9sd) SED_smoothed.txt',
#                     sep=" ", comment='#', header=None, names=["w", "f", "err"])
# df_1425 = pd.read_csv('Data/1425+7102 (M8sd) SED.txt', sep=" ", comment='#', header=None, names=["w", "f", "err"])
df_LHS = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/Gaia1439+1839 (M7sd) SED_smoothed.txt', sep=" ",
                     comment='#', header=None, names=["w", "f", "err"])
df_1444 = pd.read_csv('Data/1444-2019 (M9sd) SED.txt', sep=" ", comment='#', header=None, names=["w", "f", "err"])
df_1610 = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/Gaia1610-0040 (M7sd) SED_smoothed.txt', sep=" ",
                      comment='#', header=None, names=["w", "f", "err"])
df_1626 = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/Gaia1626+3925 (L4sd) SED_smoothed.txt',
                      sep=" ", comment='#', header=None, names=["w", "f", "err"])
df_2036 = pd.read_csv('Data/Smoothed_data/Eileen_spectra_output/Subdwarfs_corrected/2036+5059 (M7.5sd) SED_smoothed.txt', sep=" ",
                      comment='#', header=None, names=["w", "f", "err"])
...
# ------- Not enough detail if in one figure => make two  Ms and Ls------------------
...
ax1.plot(df_0616['w'], norm_df_0616 + 1, c='darkviolet')                        # sdL5 1731
ax1.plot(df_1626['w'], norm_df_1626 + 2, c='#531CF7')                           # sdL4 2158
ax1.plot(df_1256['w'], norm_df_1256 + 3, c='k')                                 # sdL3.5  2344
...
ax1.annotate('K$\,$I', xy=(0.74, 4.3), color='k', fontsize=12)
...
ax1.annotate('CrH', xy=(0.85, 4.3), color='k', fontsize=12)
...
ax1.annotate('CrH, FeH', xy=(0.996, 4.38), color='k', fontsize=12)
...
ax1.annotate('H$_\mathrm{2} $O', xy=(1.35, 4.4), color='k', fontsize=12)

CIA = pd.DataFrame()
CIA['x'] = [1.8, 2.8]
CIA['y'] = [4.35, 4.35]
ax1.plot(CIA['x'], CIA['y'], color='k')
ax1.annotate('CIA H$_\mathrm{2} $', xy=(2, 4.4), color='k', fontsize=12)

# ------- Label Sources -------------
# not alot of space to label
ax1.text(0.73, 0.12, 'J0532+8246 (sdL7)', transform=ax1.transAxes, color='indigo', fontsize=12)
ax1.text(0.73, 0.08, '$T_\mathrm{eff}: 1677 \pm 25$ K ', transform=ax1.transAxes, color='indigo',fontsize=12)
ax1.text(0.73, 0.32, 'J0616-6407 (sdL5)', transform=ax1.transAxes, color='darkviolet', fontsize=12)
ax1.text(0.73, 0.28, '$T_\mathrm{eff}: 1720 \pm 280$ K ', transform=ax1.transAxes, color='darkviolet',fontsize=12)
ax1.text(0.73, 0.52, 'J1626+3925 (sdL4)', transform=ax1.transAxes, color='#531CF7', fontsize=12)
ax1.text(0.73, 0.48, '$T_\mathrm{eff}: 2148 \pm 14$ K', transform=ax1.transAxes, color='#531CF7', fontsize=12)
...
# -------- Add data -----------
ax1.plot(df_1444['w'], norm_df_1444, c='mediumblue')                        # sdM9 2359
ax1.plot(df_1013['w'], norm_df_1013 + 1, c='#015DF7')                         # sdM9.5 2457
ax1.plot(df_LHS['w'], norm_df_LHS + 2, c='#01A1D6')                       # sdM7 2748
...
# ------- Label Sources -------------
ax1.text(0.73, 0.11, 'J1444-2019 (sdM9)', transform=ax1.transAxes, color='mediumblue', fontsize=12)
ax1.text(0.73, 0.07, '$T_\mathrm{eff}: 2477 \pm 13$ K', transform=ax1.transAxes, color='mediumblue', fontsize=12)
ax1.text(0.72, 0.29, 'J1013-1356 (sdM9.5)', transform=ax1.transAxes, color='#015DF7', fontsize=12)
ax1.text(0.72, 0.25, '$T_\mathrm{eff}: 2621 \pm 22$ K', transform=ax1.transAxes, color='#015DF7', fontsize=12)
ax1.text(0.72, 0.48, 'LHS 377 (sdM7)', transform=ax1.transAxes, color='#01A1D6',  fontsize=12)
ax1.text(0.72, 0.44, '$T_\mathrm{eff}: 2839 \pm 6$ K', transform=ax1.transAxes, color='#01A1D6', fontsize=12)
